  ...
  function updateListItemEffects(ns, meas) {
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (useKind === 'idx2') {
        ...
        // снизу (index+3), при движении от bottomBandEnd→bottomBandStart:
        //   scale 0.92→0.79, opacity 0→0, bg 18→14
        const s = 0.79 + 0.13 * useP; // при p:1→0 даёт 0.92→0.79
        const o = 0;
        const r = Math.round(color14.r + (color18.r - color14.r) * useP); // при p:1→0 даёт 18→14
        ...
        // снизу (index+2), при движении от bottomBandEnd→bottomBandStart:
        //   scale 1→0.92, opacity 1→0, bg 21→18
        const s = 0.92 + 0.08 * useP; // при p:1→0 даёт 1→0.92
        const o = useP;               // при p:1→0 даёт 1→0
        const r = Math.round(color18.r + (color21.r - color18.r) * useP); // при p:1→0 даёт 21→18
